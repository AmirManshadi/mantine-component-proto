...
A React 19 + TypeScript + Vite component showcase application demonstrating modern development patterns with Mantine UI, TanStack Router, Tailwind CSS, and PostCSS. **Hub**: `src/routes/index.ts` (all route definitions), `src/config/constants.ts` (centralized config), `src/components/ComponentShowcase.tsx` (DRY template).
...
│   └── theme.ts         (Mantine theme with PRIMARY_COLOR)
...
- **Theme**: `src/config/theme.ts` defines Mantine theme (primary color `0x0066cc`)
